...
window.CH[6] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│           DISTRIBUTED TRACING — BOTTLENECK HUNT              │\n├──────────────────────────────────────────────────────────────┤\n│  [User Request] TraceID: abc123                              │\n│       |                                                      │\n│  [API Gateway]    Span 1:   2ms  --> Jaeger / Tempo          │\n│       | propagate W3C traceparent header downstream         │\n│  [Auth Service]   Span 2:   5ms                              │\n│       |                                                      │\n│  [Inventory Svc]  Span 3: 120ms  <-- BOTTLENECK!            │\n│       |                                                      │\n│  [PostgreSQL]     Span 4: 115ms  <-- missing index!         │\n│                                                              │\n│  Without tracing: "The API is slow"  (no actionable data)   │\n│  With tracing: "Line 42 DB query = 115ms -- add index"      │\n│                                                              │\n│  Sampling strategy:                                          │\n│    1%  of healthy traffic (cost control)                    │\n│    100% of errors + high-latency outliers                   │\n│                                                              │\n│  SENIOR: Head-based vs tail-based sampling tradeoffs        │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>The error budget\'s real power is as an organizational contract, not a math exercise: it converts "should we ship this risky change" from ' +
