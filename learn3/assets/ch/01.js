...
window.CH[1] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│          KUBERNETES CRD + OPERATOR RECONCILE LOOP            │\n├──────────────────────────────────────────────────────────────┤\n│  1. Apply CRD YAML --> API Server registers new kind        │\n│  2. User creates CR: kind: MyDatabase --> etcd stores spec  │\n│  3. Operator Informer detects AddFunc/UpdateFunc event      │\n│  4. RECONCILE LOOP:                                          │\n│       desired (CR spec)  vs  actual (cluster state)         │\n│       --> Create / Update / Delete child resources           │\n│  5. Update CR status.conditions:                             │\n│       Ready: True / Reason: DatabaseRunning                  │\n│                                                              │\n│  INSIGHT: Operators = human operational knowledge as code   │\n│  NO manual runbooks -- the operator handles Day-2 ops       │\n│                                                              │\n│  Senior Pattern:                                             │\n│    - Use controller-runtime / kubebuilder scaffold           │\n│    - Implement Status.Conditions (not just booleans)        │\n│    - Exponential backoff on reconcile errors                │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, understand that the Kubernetes API server is a generic, declarative <b>resource store with a watch API</b> — and a CRD is ' +
