...
window.CH[4] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│            GITOPS DELIVERY PIPELINE (ArgoCD)                 │\n├──────────────────────────────────────────────────────────────┤\n│                                                              │\n│  Dev --> Git PR --> CI tests pass --> Merge to main         │\n│                             |                                │\n│              +---------- ArgoCD -----------+                 │\n│              | polls Git every 3 min       |                 │\n│              +-----------------------------+                 │\n│                             |                                │\n│           diff: desired (Git YAML)                           │\n│                 vs actual  (K8s cluster state)               │\n│                             | drift detected                │\n│                             v                                │\n│              kubectl apply (sync to Git state)               │\n│                             |                                │\n│              Cluster converges to Git - done!               │\n│                                                              │\n│  SENIOR: Sync waves + resource hooks for ordered deploys    │\n│  ApplicationSet for multi-cluster / multi-tenant GitOps     │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>Expert GitOps design is really about where you draw <b>Application/Kustomization boundaries</b>: too coarse and one team\'s bad commit ' +
