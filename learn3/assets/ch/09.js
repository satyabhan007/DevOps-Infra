...
window.CH[9] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│               KUBERNETES RBAC DECISION CHAIN                 │\n├──────────────────────────────────────────────────────────────┤\n│  $ kubectl get pods  (sa: ci-deployer)                       │\n│              |                                               │\n│    +-------- v --------+                                     │\n│    | Authentication    |  ServiceAccount JWT verified       │\n│    +---------+---------+                                     │\n│              |                                               │\n│    +-------- v --------+                                     │\n│    | Authorization     |  RoleBinding -> Role checked       │\n│    | (RBAC check)      |  Rules: verbs=[get,list] on pods  │\n│    +---------+---------+                                     │\n│              |                                               │\n│    +-------- v --------+                                     │\n│    | Admission Control |  Kyverno / OPA Gatekeeper          │\n│    | (validating whs.) |  e.g. must set resource limits    │\n│    +---------+---------+                                     │\n│              |                                               │\n│        ALLOW  or  DENY (403 + audit log entry)              │\n│                                                              │\n│  SENIOR: Audit all DENY events; alert on spikes in RBAC err │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>Expert autoscaler design treats node-pool shape as a <b>direct lever on both reliability and cost</b>: too few, too-generic pools mean slow, ' +
