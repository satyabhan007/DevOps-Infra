...
window.CH[5] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│              LEAST-PRIVILEGE IAM POLICY PATTERN              │\n├──────────────────────────────────────────────────────────────┤\n│  Step 1: Identify EXACT API calls the workload needs        │\n│          e.g. s3:GetObject ONLY — not s3:* or s3:PutObject  │\n│                                                              │\n│  Step 2: Scope to minimum resources:                         │\n│    ❌ arn:aws:s3:::*           all buckets — WRONG           │\n│    ✅ arn:aws:s3:::app-bucket/* one bucket — RIGHT           │\n│                                                              │\n│  Step 3: Add Conditions:                                     │\n│    aws:SourceVpc: vpc-0abc1234  VPC-scoped access only      │\n│    aws:MultiFactorAuthPresent: true  MFA for human access   │\n│                                                              │\n│  Step 4: Audit with IAM Access Analyzer                     │\n│    → Remove unused permissions quarterly                    │\n│                                                              │\n│  SENIOR TIP: Deny-by-default at Org SCP level first,        │\n│  then targeted allow policies per workload identity         │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, IAM is the <b>single highest-leverage control</b> in cloud security — most real breaches are a credential or a ' +
