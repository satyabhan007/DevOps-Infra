...
window.CH[7] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│              VAULT DYNAMIC SECRETS LIFECYCLE                 │\n├──────────────────────────────────────────────────────────────┤\n│  App Pod       Vault Server          PostgreSQL DB           │\n│     |               |                     |                  │\n│  1. JWT Auth  ----> |                     |                  │\n│  2. DB creds req -> |                     |                  │\n│                     |---> CREATE ROLE --->|                  │\n│                     |     user_x1f_1h     |                  │\n│               <---- |                     |                  │\n│  3. Temp creds      |                     |                  │\n│     TTL: 1 hour     |                     |                  │\n│  4. Connect -------------------------------> (authenticated) │\n│  5. TTL expires     |---> REVOKE ROLE --->|                  │\n│                                                              │\n│  Result: credentials NEVER appear in config files            │\n│  Rotation: Fully automatic (Vault-managed)                  │\n│  Audit: Every credential request logged to Vault audit log  │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, secrets management is about shrinking the <b>window of usefulness</b> of any credential that leaks, rather than trying ' +
