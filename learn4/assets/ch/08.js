...
window.CH[8] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│              mTLS MUTUAL AUTH HANDSHAKE                      │\n├──────────────────────────────────────────────────────────────┤\n│  Service A (Client)              Service B (Server)         │\n│       |                               |                      │\n│  1. --> ClientHello                   |                      │\n│  2.  <- ServerHello + Cert ---------- |                      │\n│  3. Verify Server Cert (CA chain) ... |                      │\n│  4. --> Client Cert ----------------> |                      │\n│  5.  <- Verify Client Cert --------- |                      │\n│  6. Encrypted session established OK |                      │\n│                                       |                      │\n│  cert-manager automates this:         |                      │\n│  Certificate CR --> Vault/ACME Issuer |                      │\n│  --> Signed cert injected into Pod secret                   │\n│  Auto-renew at 80% TTL (zero human intervention)            │\n│                                                              │\n│  SENIOR: Use SPIFFE/SPIRE for zero-touch workload identity  │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, certificate management is <b>PKI operations</b> — you\'re not just renewing certs, you\'re running a trust hierarchy: ' +
