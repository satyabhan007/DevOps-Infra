...
window.CH[16] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│            SECURE CLOUD LANDING ZONE ARCHITECTURE            │\n├──────────────────────────────────────────────────────────────┤\n│  Management Account (Org Root)                               │\n│  |-- Audit/Log Archive  <-- CloudTrail, GuardDuty logs      │\n│  |-- Security Tooling   <-- Security Hub, AWS Config        │\n│  |-- Landing Zone Accounts                                   │\n│       |-- Network Account: Transit GW, shared VPC topology  │\n│       |-- Production Account:                                │\n│       |    3-Tier VPC --> IAM (PoLP) roles per workload     │\n│       |    Vault --> KMS --> S3 (encrypted, no public ACL)  │\n│       |    WAF --> ALB --> Private Subnets                  │\n│       |    cert-manager (mTLS) + Service Mesh               │\n│       |-- Staging (identical isolation, separate blast rad)  │\n│                                                              │\n│  Org SCPs: deny public S3, require MFA, restrict regions    │\n│  CIS Benchmark: automated via AWS Config conformance packs  │\n│  Compliance: 100% automated evidence, no scrambling audits  │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, a landing zone is the organizational answer to every failure mode this part walked through individually: the CIDR that ' +
