...
window.CH[7] = {
  levels: [
    ...
    /* ---------- L5 · Expert ---------- */
    { html:
      '<div class="diagram-block"><pre class="arch-diagram">┌──────────────────────────────────────────────────────────────┐\n│              CI/CD FOR TERRAFORM — GITOPS FLOW               │\n├──────────────────────────────────────────────────────────────┤\n│  PR Opened:                                                  │\n│  (1) terraform fmt --check     style enforcement            │\n│  (2) terraform validate        schema check                 │\n│  (3) tflint                    best practices lint          │\n│  (4) checkov / tfsec           security: no open SGs        │\n│  (5) terraform plan  --> post diff as PR comment            │\n│                                                              │\n│  PR Merged to main:                                          │\n│  (6) terraform apply --auto-approve                         │\n│  (7) Post-apply: terratest / conftest assertions            │\n│  (8) Notify Slack / PagerDuty on failure                    │\n│                                                              │\n│  State backend: S3 + DynamoDB lock table                    │\n│  Workspaces: prod / staging / dev (separate state files)    │\n│                                                              │\n│  SENIOR: Use plan-then-apply with PR approval gating        │\n│  Never run terraform apply locally against prod             │\n└──────────────────────────────────────────────────────────────┘\n</pre></div><p>At expert level, secrets in IaC is a <b>trust chain</b> problem end to end: the CI runner\'s own identity (ideally short-lived, OIDC-federated, ' +
